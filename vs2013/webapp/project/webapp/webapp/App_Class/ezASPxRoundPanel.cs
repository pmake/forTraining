        ...
        public static void Init(object sender, EventArgs e, bool bHeaderText)
        {
            if (bHeaderText)
            {
                ...
                if (string.IsNullOrEmpty(strPrgNo))
                {
                    ...
                    ezsql.ParametersAdd("@url_prg", str_url_prg, true);
                    if (ezsql.HasRows)
                    {
                        int int_count = ezsql.GetValueInt("counts");
                        if (int_count == 1)
                        {
                            ezsql.CommandText = "SELECT no_prg , name_prg FROM z_sys_prg WHERE url_prg like @url_prg";
                            ezsql.ParametersAdd("@url_prg", str_url_prg, true);
                            ezSession.PrgNo = ezsql.GetValueString("no_prg");
                            ezSession.PrgName = ezsql.GetValueString("name_prg");
            ...
            (sender as ASPxRoundPanel).Font.Size = FontUnit.Medium;
