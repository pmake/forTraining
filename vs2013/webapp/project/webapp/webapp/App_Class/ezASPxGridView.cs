...
namespace ezapp
{
    ...
    public static class ezASPxGridView
    {
        ...
        public static void Init(object sender, EventArgs e, bool bSecurity)
        {
            ...
            (sender as ASPxGridView).Font.Size = FontUnit.Medium;
            ...
            (sender as ASPxGridView).SettingsPopup.EditForm.Width = Unit.Pixel(800);
        ...
        {
            CheckRequire(sender, e, sDataColName, "");
        }
        ...
        public static void CheckRequire(object sender, DevExpress.Web.Data.ASPxDataValidationEventArgs e, string sDataColName, string sErrorText)
        {
            if (string.IsNullOrEmpty(sErrorText)) sErrorText = "此欄位不可空白!!";
            if (e.NewValues[sDataColName] == null) AddError(e.Errors, (sender as ASPxGridView).Columns[sDataColName], sErrorText);
        }
        ...
        {
            CheckDuplicate(sender, e, sDataColName, sTableName, sColName, "", "");
        }
        ...
        public static void CheckDuplicate(object sender, DevExpress.Web.Data.ASPxDataValidationEventArgs e, string sDataColName, string sTableName, string sColName, string sErrorText)
        {
            CheckDuplicate(sender, e, sDataColName, sTableName, sColName, "", sErrorText);
        }
        /// <summary>
        /// 檢查輸入欄位是否重覆
        /// </summary>
        ...
        /// <param name="sErrorText">錯誤訊息 , 空白為顯示 [不可重覆輸入!!]</param>
        public static void CheckDuplicate(object sender, DevExpress.Web.Data.ASPxDataValidationEventArgs e, string sDataColName, string sTableName, string sColName, string sWhereString, string sErrorText)
        {
            int int_data = 0;
            string str_data = "";
            string str_type = e.NewValues[sDataColName].GetType().Name;
            ...
            if (str_type == "String") ezsql.ParametersAdd("@new_value", str_data, true);
            if (str_type == "Int32")
            {
                if (string.IsNullOrEmpty(str_data)) str_data = "0";
                int_data = int.Parse(str_data);
                ezsql.ParametersAdd("@new_value", int_data, true);
            }
            //修改時要確認不要判斷到同筆,避免誤判
            if (!e.IsNewRow)
            {
                ...
                if (str_type == "String") ezsql.ParametersAdd("@old_value", str_data, false);
                if (str_type == "int")
                {
                    if (string.IsNullOrEmpty(str_data)) str_data = "0";
                    int_data = int.Parse(str_data);
                    ezsql.ParametersAdd("@old_value", int_data, false);
                }
            }
            if (!string.IsNullOrEmpty(sWhereString)) ezsql.CommandText += "AND " + sWhereString;
            bool bln_hasrow = ezsql.HasRows;
            ezsql.Close();
            if (bln_hasrow) AddError(e.Errors, (sender as ASPxGridView).Columns[sDataColName], sErrorText);
        }
